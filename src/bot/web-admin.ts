...
import { isPublicWebBaseUrlSecure } from './web-url';
...
const qlStatusSnapshotProvider = createQlStatusSnapshotProvider(async () => {
...
const setSessionCookie = (res: express.Response, token: string) => {
  const secure = isPublicWebBaseUrlSecure(process.env) || process.env.ADMIN_COOKIE_SECURE === 'true';
  const parts = [
...
app.get('/api/status', async (req, res) => {
  ...
  const qlStatus = await qlStatusSnapshotProvider.getSnapshot();
  ...
  res.json({
    ...
    ql_api_ready: qlStatus.qlApiReady,
    ql_api_checked_at: qlStatus.updatedAt
  });
});
