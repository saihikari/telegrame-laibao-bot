...
const ad_action_receipt_1 = require("./ad-action-receipt");
const telegram_bot_actions_1 = require("./telegram-bot-actions");
...
const storeQuerySessions = new Map();
  ...
  商户查询 - 唤起经理列表，导出最近 1 个月登记的商户信息(CSV)
    ...
    bot.onText(/昨日消耗/, async (msg) => {
        ...
        try {
            ...
            const rowsRaw = await ql_api_1.qlApi.listSumShowByDateRange({
                managerBId: 579,
                startStr: dateStr,
                endStr: dateStr,
                pageNum: 1,
            ...
            const rows = (rowsRaw || [])
                .filter((r) => r && r.storeName && r.productName && r.productName !== '合计')
                .map((r) => ({
            ...
            await bot.sendDocument(msg.chat.id, Buffer.from(csv, 'utf8'), { caption: `昨日消耗 (${dateStr})，共 ${rows.length} 行` }, { filename, contentType: 'text/csv' });
    ...
    const buildOfferKeyboard = (activeOffers, selectedOffers, columns) => {
        const keyboard = [];
        let currentRow = [];
    ...
    const getStoreKeyboard = (callbackPrefix = 'charge_store') => {
        const config = (0, config_loader_1.getConfig)();
    ...
    bot.onText(/(暂停|开启|下架)广告/, async (msg) => {
        ...
        if (/^(?:\/)?(暂停|开启|下架)广告$/.test(text))
            return; // handed by exact match above
        const lines = text.split('\n');
        const tasks = [];
        for (const line of lines) {
            const trimLine = line.trim();
            if (!trimLine || /^(?:\/)?(暂停|开启|下架)广告$/.test(trimLine))
                continue;
            let match = trimLine.match(/(.*?)\s*(暂停|开启|下架)广告/);
            if (match && match[1].trim()) {
                tasks.push({ productName: match[1].trim(), action: match[2] });
                continue;
            }
            match = trimLine.match(/(暂停|开启|下架)广告\s*(.*)/);
            if (match && match[2].trim()) {
                tasks.push({ productName: match[2].trim(), action: match[1] });
                continue;
            }
        }
        if (tasks.length === 0)
    ...
    bot.on('message', async (msg) => {
        ...
        if (pauseAdSession && pauseAdSession.step === 'WAIT_STORE_NAME_MANUAL' && msg.text && !msg.text.startsWith('/')) {
            ...
            try {
                ...
                const activeOffers = (0, telegram_bot_actions_1.filterOffersForAdAction)(offers, actionType);
    ...
    bot.on('callback_query', async (query) => {
        ...
        if (data?.startsWith('adaction_store:')) {
            ...
            if (storeName === 'MANUAL') {
                ...
                ql_api_1.qlApi.listStoreToSelect().then(stores => {
                    const targetStore = stores.find(s => s.storeName.includes(storeName));
                    if (!targetStore)
                        throw new Error(`找不到包含 '${storeName}' 的商户`);
            ...
            const processQuery = async () => {
                try {
                    ...
                    for (let i = 1; i <= 20; i++) {
                        ...
                        const res = await ql_api_1.qlApi.qlFetch(url, { method: 'GET', useStaticToken: true });
                ...
                try {
                    ...
                    if (pausedOffers.length > 0) {
                        ...
                        try {
                            ...
                            managerNameById = Object.fromEntries(managers.map((manager) => [
                                String(manager.userId),
                                String(manager.nickname || manager.username || manager.userId || '').trim(),
                            ]));
                        }
                        catch (managerErr) {
                            console.warn(`[Ad Action] 获取投手昵称失败: ${managerErr.message}`);
                        }
                        const receiptText = (0, ad_action_receipt_1.buildAdActionSuccessReceipt)(session.actionType, session.storeName || '未知商户', pausedOffers, managerNameById);
                        ...
                        await bot.editMessageText(`❌ 操作失败，本次成功 0 个，失败 ${failCount} 个`, {
                            chat_id: msg.chat.id,
                            message_id: msg.message_id
                        });
                    }
        ...
        if (data === 'record_no') {
            ...
            try {
                ...
                const qlResult = await (0, ql_writer_1.processAndWriteToQL)(recordsToProcess, startAtMs);
    ...
    bot.onText(/^(?:\/)?(?:audit_jwt|安全体检)$/, async (msg) => {
        try {
            const token = ql_api_1.qlApi.currentToken;
            if (!token) {
